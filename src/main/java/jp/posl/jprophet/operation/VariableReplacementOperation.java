...
public class VariableReplacementOperation implements AstOperation {
    ...
    private List<String> collectFieldNames(Node node) {
        ...
        final List<String> fieldNames = fields.stream().map(field -> field.getName().asString())
    ...
    private List<String> collectLocalVarNames(Node node) {
        ...
        final List<String> localVarNames = localVars.stream().map(localVar -> localVar.getName().asString())
                .collect(Collectors.toList());
    ...
    private List<String> collectParameterNames(Node node) {
        ...
        final List<String> parameterNames = parameters.stream().map(localVar -> localVar.getName().asString())
                .collect(Collectors.toList());
     ...
     */
    private List<DiffWithType> replaceVariables(Node targetNode, Function<String, Expression> constructVar,
     ...
     * @return 置換によって生成された修正後のCompilationUnitのリスト
     */
    private List<DiffWithType> replaceAssignExpr(Node targetNode, List<String> varNames,
        ...
        if (targetNode instanceof AssignExpr) {
            ...
            for (String varName : varNames) {
                ...
                // NodeUtility.replaceNode(constructExpr.apply(varName),
                // ((AssignExpr)targetNode).getValue())
                // .flatMap(n -> n.findCompilationUnit())
                // .ifPresent(candidates::add);
                DiffWithType candidate = new DiffWithType(DiffWithType.ModifyType.CHANGE,
                        ((AssignExpr) targetNode).getValue(), constructExpr.apply(varName));
     ...
     * @return 置換によって生成された修正後のCompilationUnitのリスト
     */
    private List<DiffWithType> replaceArgs(Node targetNode, List<String> varNames,
            Function<String, Expression> constructExpr) {
        List<DiffWithType> candidates = new ArrayList<DiffWithType>();
        ...
        if (targetNode instanceof MethodCallExpr) {
            ...
            for (String varName : varNames) {
                for (Expression arg : args) {
                    ...
                    // .flatMap(n -> n.findCompilationUnit())
                    // .ifPresent(candidates::add);
                    DiffWithType candidate = new DiffWithType(DiffWithType.ModifyType.CHANGE, arg,
     ...
     * @return 置換によって生成された修正後のCompilationUnitのリスト
     */
    private List<DiffWithType> replaceNameExprInIfCondition(Node targetNode, List<String> varNames,
            Function<String, Expression> constructExpr) {
        List<DiffWithType> candidates = new ArrayList<DiffWithType>();
        ...
        if (targetNode instanceof IfStmt) {
            ...
            for (NameExpr var : varsInCondition) {
                for (String varName : varNames) {
                    ...
                    // .flatMap(n -> n.findCompilationUnit())
                    // .ifPresent(candidates::add);
                    DiffWithType candidate = new DiffWithType(DiffWithType.ModifyType.CHANGE, var,
                            constructExpr.apply(varName));
                    candidates.add(candidate);
                }
            }
     ...
     * @return 置換によって生成された修正後のCompilationUnitのリスト
     */
    private List<DiffWithType> replaceVarInReturnStmt(Node targetNode, List<String> varNames,
            Function<String, Expression> constructExpr) {
        List<DiffWithType> candidates = new ArrayList<DiffWithType>();
